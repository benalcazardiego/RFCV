...
def VnaMeasure(ui, ip, port):
	...
	for idx, spar in enumerate([SParameters.S11, SParameters.S12, SParameters.S21, SParameters.S22]):
        ...
        retrieve_data(ip, port, f, fmat, channel.executor,spar) #Get the data
	while True:
            vna_ready = channel.is_ready()  
            if vna_ready:
                break
	...
	for ch, sparam in zip([1,2,3,4], [SParameters.S11, SParameters.S12, SParameters.S21, SParameters.S22]):
		channel.set_sparam(sparam, sparam) #Set the four parameters
...
def retrieve_data(ip, port, fname, fmat, executor,spar):
    #Function "retrieve_data". Summary goes here:
    #
    ...
    executor.execute_command(":FORM:DATA ASC") # Set data to ASCII
    template = ":CALC{ch}:TRAC{trace}:DATA:FDAT?"
    data = executor.ask(template.format(ch=str(1),trace=str(spar)))
    #data = executor.ask(":CALC{ch}:DATA:FDAT?".format(ch=str(spar))) # Call the VNA and make the respective operation
    ...
    if len(sdata) == 4: #Check if the 4 parametres of the 'S Matrix' were measured
        ...
        save_data(sdata,freq_data,fname)
        '''
